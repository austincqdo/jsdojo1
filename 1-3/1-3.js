...
for (const p in fileManage) {
    if (fileManage[p]['View Auth'] === 'Everyone') {
        allAccess.push(fileManage[p]['Folder Name']);
    }
}
...
for (let i = 0; i < allAccess.length; i++) {
    ...
    const textnode = document.createTextNode(allAccess[i]);
